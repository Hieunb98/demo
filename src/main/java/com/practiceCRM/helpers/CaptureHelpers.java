...
public class CaptureHelpers extends ScreenRecorder {
    ...
    public static void captureScreenshot(WebDriver driver, String screenName, String status) {
        try {
            String dateFolder = new SimpleDateFormat("yyyy-MM-dd").format(new Date());
            String path = SystemHelpers.getCurrentDir() + FrameworkConstants.EXPORT_CAPTURE_PATH + File.separator + dateFolder;
            if (status != null && !status.isEmpty()) {
                path = path + File.separator + status;
            }
            File file = new File(path);
            if (!file.exists()) {
                file.mkdirs();
                LogUtils.info("Created screenshot date folder: " + file);
            }

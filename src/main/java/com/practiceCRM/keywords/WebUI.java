...
import com.practiceCRM.utils.DateUtils;
...
/**
 * Keyword WebUI is a generic class that is a preprocessed library with many
 * custom functions from Selenium and Java.
 * Returns is a Class containing Static functions. Callback is used by taking
 * the class name and dotting the function name (WebUI.method)
 */
public class WebUI {

   /**
    * The SoftAssert object is created and managed by ThreadLocal for parallel
    * execution
    */
   ...
   public static void stopSoftAssertAll() {
      try {
         ...
         softAssert.remove(); // Xóa instance sau khi assert xong để dọn dẹp cho test kế tiếp
      }
   }

   /**
    * Clear the Soft Assert ThreadLocal without throwing exceptions
    */
   ...
   public static void addScreenshotToReport(String screenName) {
      if (FrameworkConstants.SCREENSHOT_ALL_STEPS.equals(FrameworkConstants.YES)) {
         ...
         // CaptureHelpers.captureScreenshot(DriverManager.getDriver(),
         // Helpers.makeSlug(screenshotName));
         AllureManager.takeScreenshotStep();
      }
   }

   /**
    * Take a screenshot of a specific web element. The captured image will be saved
    * in '.png' format.
    *
    * @param screenName Screenshot name
    */
   public static void takeElementScreenshot(By by, String screenName) {
      ...
      try {
         String path = SystemHelpers.getCurrentDir() + FrameworkConstants.EXPORT_CAPTURE_PATH + File.separator
               + dateFolder;
         ...
         // result.getName() gets the name of the test case and assigns it to the
         // screenshot file name
         FileUtils.copyFile(source,
               new File(path + File.separator + screenName + "_" + dateFormat.format(new Date()) + ".png"));
   ...
   /**
    * Take entire-page screenshot, including overflow parts. The captured image
    * will be saved in '.png' format.
    * This method simulates a scroll action to take a number of shots and merge
    * them together to make a full-page screenshot.
    *
    * @param screenName Screenshot name
    */
   ...
   /**
    * Dọn sạch toàn bộ các file trong thư mục tải về (target/downloads).
    */
   public static void cleanDownloadDirectory() {
      try {
         String path = getPathDownloadDirectory();
         ...
         LogUtils.info("🧹 Cleaned download directory: " + path);
      } catch (Exception e) {
         LogUtils.warn("Cannot clean download directory: " + e.getMessage());
      }
   }

   /**
    * Xóa một file cụ thể trong thư mục tải về nếu tên chứa fileName.
    *
    * @param fileName Tên hoặc một phần tên file cần xóa
    */
            ...
            if (files != null) {
               for (File f : files) {
                  if (f.isFile() && f.getName().contains(fileName)) {
                     f.delete();
                     LogUtils.info("Deleted downloaded file: " + f.getName());
                  }
               }
            }
         }
      } catch (Exception e) {
         LogUtils.warn("Cannot delete downloaded file: " + e.getMessage());
      }
   }
    ...
    *
    * @param fileName       the specified file
    * @param timeoutSeconds System will wait at most timeout (seconds) to return
    *                       result
   ...
   public static ChromeOptions notificationsAllow() {
      ...
      // Use the setExperimentalOption function to execute the value through the above
      // prefs object
   ...
   /**
    * Uploading files with a click shows a form to select local files on your
    * computer
    *
    * @param by       is an element of type By
    ...
    */
   @Step("Upload File with SendKeys: {1}")
   public static void uploadFileWithSendKeys(By by, String filePath) {
      smartWait();
      File file = new File(filePath);
      String absolutePath = file.getAbsolutePath();
      waitForElementPresent(by).sendKeys(absolutePath);

      LogUtils.info("Upload File with SendKeys: " + absolutePath);
      if (ExtentTestManager.getExtentTest() != null) {
         ExtentReportManager.info("Upload File with SendKeys: " + absolutePath);
      }
      AllureManager.saveTextLog("Upload File with SendKeys: " + absolutePath);
   }
   ...
   public static boolean verifySelectedByValue(By by, String value) {
      ...
      if (select.getFirstSelectedOption().getAttribute("value").equals(value)) {
         ...
         Assert.assertEquals(select.getFirstSelectedOption().getAttribute("value"), value,
               "The option NOT selected. " + by);
         return false;
      }
   }
   ...
   public static boolean verifyTotalOfWindowsOrTab(int number) {
      return new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(FrameworkConstants.WAIT_EXPLICIT))
   ...
   public static void switchToLastWindow() {
      ...
      WebDriver newDriver = DriverManager.getDriver().switchTo()
   ...
   public static void acceptAlert() {
      verifyAlertPresent(FrameworkConstants.WAIT_EXPLICIT);
   ...
   public static boolean verifyTrue(boolean condition, String message) {
      if (condition) {
         ...
         AllureManager.saveTextLog("❌ Verify TRUE: " + condition + (message != null ? " - " + message : ""));
   ...
   /**
    * Verify the condition is true with FailureHandling (Tính đa hình - Method
    * Overloading).
    *
    * @param condition   the boolean condition
    * @param message     the custom message if false
    * @param flowControl Specify failure handling schema (STOP_ON_FAILURE,
    *                    CONTINUE_ON_FAILURE, OPTIONAL)
    * @return true/false
    */
   @Step("Verify TRUE: {0} with FailureHandling")
   public static boolean verifyTrue(boolean condition, String message, FailureHandling flowControl) {
      if (condition) {
         LogUtils.info("Verify TRUE: " + condition);
         if (ExtentTestManager.getExtentTest() != null) {
            ExtentReportManager.pass("Verify TRUE: " + condition);
         ...
         String errorMsg = (message == null || message.trim().isEmpty()) ? "The condition is FALSE." : message;
         LogUtils.error("❌ " + errorMsg);
         AllureManager.saveTextLog("❌ " + errorMsg);
         ...
         if (flowControl.equals(FailureHandling.STOP_ON_FAILURE)) {
            ...
            softAssert.get().assertTrue(condition, errorMsg);
            if (ExtentTestManager.getExtentTest() != null) {
               ExtentReportManager.fail(errorMsg);
            }
         } else if (flowControl.equals(FailureHandling.OPTIONAL)) {
            if (ExtentTestManager.getExtentTest() != null) {
               ExtentReportManager.warning(errorMsg);
            }
         }
      }
      return condition;
    ...
    * @param text        Text of the element to verify.
    * @param flowControl Specify failure handling schema (STOP_ON_FAILURE,
    *                    CONTINUE_ON_FAILURE, OPTIONAL) to determine whether the
    *                    execution should be allowed to continue or stop
    * @return true if the element has the desired text, otherwise false.
    */
   ...
   public static boolean verifyElementTextEquals(By by, String text, FailureHandling flowControl) {
      ...
      if (flowControl.equals(FailureHandling.CONTINUE_ON_FAILURE)) {
         softAssert.get().assertEquals(getTextElement(by).trim(), text.trim(),
               "❌ The actual text is '" + getTextElement(by).trim() + "' not equals '" + text.trim() + "'");
         if (!result) {
            ExtentReportManager
                  .fail("The actual text is '" + getTextElement(by).trim() + "' not equals '" + text.trim() + "'");
         }
      }
      if (flowControl.equals(FailureHandling.OPTIONAL)) {
         if (ExtentTestManager.getExtentTest() != null) {
            ExtentReportManager.warning("Verify text of an element [Equals] - " + result);
            ExtentReportManager.warning("The actual text is '" + getTextElement(by).trim()
                  + "' not equals expected text '" + text.trim() + "'");
         }
         AllureManager.saveTextLog("Verify text of an element [Equals] - " + result + ". The actual text is '"
               + getTextElement(by).trim() + "' not equals '" + text.trim() + "'");
      }
   ...
   public static boolean verifyElementTextEquals(By by, String text) {
      ...
      if (ExtentTestManager.getExtentTest() != null) {
         ExtentReportManager.warning("Verify text of an element [Equals] : " + result);
         ExtentReportManager
               .warning("The actual text is '" + getTextElement(by).trim() + "' not equals '" + text.trim() + "'");
      }
      AllureManager.saveTextLog("Verify text of an element [Equals] : " + result + ". The actual text is '"
            + getTextElement(by).trim() + "' NOT equals '" + text.trim() + "'");
   ...
   public static boolean verifyElementClickable(By by) {
      return verifyElementClickable(by, FrameworkConstants.WAIT_EXPLICIT, "❌ Element not clickable " + by,
   ...
   public static boolean verifyElementClickable(By by, int timeout) {
      return verifyElementClickable(by, timeout, "❌ Element not clickable " + by, FailureHandling.STOP_ON_FAILURE);
   }

   /**
    * Verify if the given element is clickable with custom message.
    *
    * @param by      Represent a web element as the By object
    * @param message the custom message if false
    * @return true/false
   ...
   public static boolean verifyElementClickable(By by, String message) {
      return verifyElementClickable(by, FrameworkConstants.WAIT_EXPLICIT, message, FailureHandling.STOP_ON_FAILURE);
   }

   /**
    * Verify if the given element is clickable with FailureHandling.
    *
    * @param by          Represent a web element as the By object
    * @param flowControl FailureHandling (STOP_ON_FAILURE, CONTINUE_ON_FAILURE,
    *                    OPTIONAL)
    * @return true/false
    */
   @Step("Verify element Clickable {0} with FailureHandling")
   public static boolean verifyElementClickable(By by, FailureHandling flowControl) {
      return verifyElementClickable(by, FrameworkConstants.WAIT_EXPLICIT, "❌ Element not clickable " + by, flowControl);
   }

   /**
    * Verify if the given element is clickable with timeout and FailureHandling.
    *
    * @param by          Represent a web element as the By object
    * @param timeout     System will wait at most timeout (seconds) to return
    *                    result
    * @param flowControl FailureHandling (STOP_ON_FAILURE, CONTINUE_ON_FAILURE,
    *                    OPTIONAL)
    * @return true/false
    */
   @Step("Verify element Clickable {0} with timeout {1} second and FailureHandling")
   public static boolean verifyElementClickable(By by, int timeout, FailureHandling flowControl) {
      return verifyElementClickable(by, timeout, "❌ Element not clickable " + by, flowControl);
   }

   /**
    * Verify if the given element is clickable with custom message and
    * FailureHandling.
    *
    * @param by          Represent a web element as the By object
    * @param message     the custom message if false
    ...
    */
   @Step("Verify element Clickable {0} with FailureHandling")
   public static boolean verifyElementClickable(By by, String message, FailureHandling flowControl) {
      return verifyElementClickable(by, FrameworkConstants.WAIT_EXPLICIT, message, flowControl);
   }
   ...
   public static boolean verifyElementClickable(By by, int timeout, String message) {
      return verifyElementClickable(by, timeout, message, FailureHandling.STOP_ON_FAILURE);
   }
    ...
    */
   @Step("Verify element Clickable {0} with FailureHandling")
   public static boolean verifyElementClickable(By by, int timeout, String message, FailureHandling flowControl) {
      smartWait();
      ...
      try {
         ...
         String errorMsg = (message == null || message.trim().isEmpty()) ? "❌ Element not clickable " + by : message;
         LogUtils.error(errorMsg);
         AllureManager.saveTextLog(errorMsg);
   ...
   /**
    * Check if the given element is disabled (cannot be interacted).
    *
    * @param by      Represent a web element as the By object
         ...
         return wait.until(driver -> {
            try {
               ...
               return !element.isEnabled()
                     ...
                     || (disabledAttr != null
                           && (disabledAttr.equalsIgnoreCase("true") || disabledAttr.equalsIgnoreCase("disabled")))
   ...
   public static boolean isElementDisabled(By by) {
      return isElementDisabled(by, FrameworkConstants.WAIT_EXPLICIT);
   }

   /**
    * Verify if the given element is disabled with default timeout and
    * STOP_ON_FAILURE.
    *
    * @param by Represent a web element as the By object
    * @return true/false
    */
   @Step("Verify element Disabled {0}")
   public static boolean verifyElementDisabled(By by) {
      return verifyElementDisabled(by, FrameworkConstants.WAIT_EXPLICIT, "❌ Element not disabled: " + by,
            FailureHandling.STOP_ON_FAILURE);
   }

   /**
    * Verify if the given element is disabled with timeout and STOP_ON_FAILURE.
    *
    * @param by      Represent a web element as the By object
    * @param timeout System will wait at most timeout (seconds) to return result
    * @return true/false
    */
   @Step("Verify element Disabled {0} with timeout {1} second")
   public static boolean verifyElementDisabled(By by, int timeout) {
      return verifyElementDisabled(by, timeout, "❌ Element not disabled: " + by, FailureHandling.STOP_ON_FAILURE);
   }

   /**
    * Verify if the given element is disabled with timeout, message and
    * STOP_ON_FAILURE.
    *
    * @param by      Represent a web element as the By object
    * @param timeout System will wait at most timeout (seconds) to return result
    * @param message Custom message if false
    * @return true/false
    */
   @Step("Verify element Disabled {0}")
   public static boolean verifyElementDisabled(By by, int timeout, String message) {
      return verifyElementDisabled(by, timeout, message, FailureHandling.STOP_ON_FAILURE);
   }

   /**
    * Verify if the given element is disabled with custom message and
    * STOP_ON_FAILURE.
    *
    * @param by      Represent a web element as the By object
    * @param message Custom message if false
    ...
    */
   @Step("Verify element Disabled {0}")
   public static boolean verifyElementDisabled(By by, String message) {
      return verifyElementDisabled(by, FrameworkConstants.WAIT_EXPLICIT, message, FailureHandling.STOP_ON_FAILURE);
   }

   /**
    * Verify if the given element is disabled with FailureHandling.
    *
    * @param by          Represent a web element as the By object
    * @param flowControl FailureHandling (STOP_ON_FAILURE, CONTINUE_ON_FAILURE,
    *                    OPTIONAL)
    * @return true/false
    */
   @Step("Verify element Disabled {0} with FailureHandling")
   public static boolean verifyElementDisabled(By by, FailureHandling flowControl) {
      return verifyElementDisabled(by, FrameworkConstants.WAIT_EXPLICIT, "❌ Element not disabled: " + by, flowControl);
   }

   /**
    * Verify if the given element is disabled with timeout and FailureHandling.
    *
    * @param by          Represent a web element as the By object
    * @param timeout     System will wait at most timeout (seconds) to return
    *                    result
    ...
    * @return true/false
    */
   @Step("Verify element Disabled {0} with timeout {1} second and FailureHandling")
   public static boolean verifyElementDisabled(By by, int timeout, FailureHandling flowControl) {
      return verifyElementDisabled(by, timeout, "❌ Element not disabled: " + by, flowControl);
   }

   /**
    * Verify if the given element is disabled with message and FailureHandling
    * (default timeout).
    *
    * @param by          Represent a web element as the By object
    * @param message     Custom message if false
    ...
    */
   @Step("Verify element Disabled {0} with FailureHandling")
   public static boolean verifyElementDisabled(By by, String message, FailureHandling flowControl) {
      return verifyElementDisabled(by, FrameworkConstants.WAIT_EXPLICIT, message, flowControl);
   }
    ...
    */
   @Step("Verify element Disabled {0} with FailureHandling")
   public static boolean verifyElementDisabled(By by, int timeout, String message, FailureHandling flowControl) {
      smartWait();
      ...
      try {
         ...
         boolean disabled = wait.until(driver -> {
            try {
               ...
               String disabledAttr = element.getAttribute("disabled");
               String ariaDisabled = element.getAttribute("aria-disabled");
               Object jsDisabled = ((org.openqa.selenium.JavascriptExecutor) driver).executeScript(
                     "return arguments[0].disabled === true || arguments[0].hasAttribute('disabled') || (arguments[0].getAttribute('class') && arguments[0].getAttribute('class').indexOf('disabled') !== -1);",
                     element);
               return Boolean.TRUE.equals(jsDisabled)
                     || !element.isEnabled()
                     || (cls != null && cls.contains("disabled"))
                     || (disabledAttr != null
                           && (disabledAttr.equalsIgnoreCase("true") || disabledAttr.equalsIgnoreCase("disabled")))
         ...
         if (!disabled) {
            throw new RuntimeException("Element is still enabled: " + by);
         }

         LogUtils.info("Verify element disabled " + by);
         if (ExtentTestManager.getExtentTest() != null) {
            ExtentReportManager.info("Verify element disabled " + by);
         }
         AllureManager.saveTextLog("Verify element disabled " + by);
         return true;
      } catch (Exception e) {
         String errorMsg = (message == null || message.trim().isEmpty()) ? "❌ Element not disabled " + by : message;
         LogUtils.error(errorMsg);
         AllureManager.saveTextLog(errorMsg);
   ...
   /**
    * Verify if the given web element does present on DOM with FailureHandling.
    *
    * @param by          Represent a web element as the By object
    * @param timeout     System will wait at most timeout (seconds) to return
    *                    result
    ...
    */
   @Step("Verify element present {0} with timeout {1} second")
   public static boolean verifyElementPresent(By by, int timeout, String message, FailureHandling flowControl) {
      smartWait();
      ...
      try {
         ...
         String errorMsg = (message == null || message.trim().isEmpty()) ? "❌ The element does NOT present. " + by
               : message;
         LogUtils.error(errorMsg);
   ...
   /**
    * Verify if the given web element does NOT present on the DOM with
    * FailureHandling (Tính đa hình - Method Overloading).
    *
    * @param by          Represent a web element as the By object
    * @param timeout     System will wait at most timeout (seconds) to return
    *                    result
    ...
    */
   @Step("Verify element NOT present {0} with timeout {1} second")
   public static boolean verifyElementNotPresent(By by, int timeout, String message, FailureHandling flowControl) {
      smartWait();
      try {
         WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(timeout));
         wait.until(ExpectedConditions.presenceOfElementLocated(by));
         String errorMsg = (message == null || message.trim().isEmpty()) ? "❌ The element presents: " + by : message;
         LogUtils.error(errorMsg);
   ...
   /**
    * Verify if the given web element is visible with FailureHandling (Tính đa hình
    * - Method Overloading).
    *
    * @param by          Represent a web element as the By object
    * @param timeout     System will wait at most timeout (seconds) to return
    ...
    * @return true/false
    */
   @Step("Verify element visible {0} with timeout {1} second and FailureHandling")
   public static boolean verifyElementVisible(By by, int timeout, String message, FailureHandling flowControl) {
      smartWait();
      LogUtils.info("Verify element visible " + by);
      try {
         WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(timeout));
         ...
         String errorMsg = (message == null || message.trim().isEmpty()) ? "The element is NOT visible: " + by
               : message;
         LogUtils.error(errorMsg);
         if (flowControl.equals(FailureHandling.STOP_ON_FAILURE)) {
            Assert.fail(errorMsg);
   ...
   /**
    * Verify if the given web element is NOT visible with FailureHandling (Tính đa
    * hình - Method Overloading).
    *
    * @param by          Represent a web element as the By object
    * @param timeout     System will wait at most timeout (seconds) to return
    *                    result
    ...
    * @return true/false
    */
   @Step("Verify element NOT visible {0} with timeout {1} second and FailureHandling")
   public static boolean verifyElementNotVisible(By by, int timeout, String message, FailureHandling flowControl) {
      smartWait();
      LogUtils.info("Verify element NOT visible " + by);
      try {
         WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(timeout));
         ...
         String errorMsg = (message == null || message.trim().isEmpty()) ? "FAILED. The element is visible: " + by
               : message;
         LogUtils.error(errorMsg);
         if (flowControl.equals(FailureHandling.STOP_ON_FAILURE)) {
            Assert.fail(errorMsg);
   ...
   public static boolean dragAndDropHTML5(By fromElement, By toElement) {
      ...
      try {
         ...
         // This place takes the current coordinates plus 120px which is the browser
         // header (1920x1080 current window)
   ...
   public static WebElement highLightElement(By by) {
      ...
      if (DriverManager.getDriver() instanceof JavascriptExecutor) {
         ((JavascriptExecutor) DriverManager.getDriver()).executeScript("arguments[0].style.border='3px solid fuchsia'",
               waitForElementVisible(by));
         // sleep(1);
   ...
   public static void setText(By by, String value) {
      ...
      if (element.getAttribute("type") != null && element.getAttribute("type").equals("password")) {
         ...
         Allure.getLifecycle().updateStep(stepResult -> {
            stepResult.getParameters().forEach(param -> {
      ...
      LogUtils.info("Set text " + loggedValue + " on " + by.toString());

      if (ExtentTestManager.getExtentTest() != null) {
         ExtentReportManager.pass("Set text " + loggedValue + " on " + by);
      }
      AllureManager.saveTextLog("Set text " + loggedValue + " on " + by);
   ...
   public static void setText(By by, String value, Keys keys) {
      ...
      LogUtils.info("Set text " + loggedValue + " on " + by + " and press key " + keys.name());

      if (ExtentTestManager.getExtentTest() != null) {
         ExtentReportManager.pass("Set text " + loggedValue + " on " + by + " and press key " + keys.name());
      }
      AllureManager.saveTextLog("Set text " + loggedValue + " on " + by + " and press key " + keys.name());
   ...
   /**
    * Simulates keystroke events at the current position, as though you typed the
    * value key-by-key.
    *
    * @param keys key on the keyboard to press
    */
   ...
   public static void clearAndFillText(By by, String value) {
      ...
      LogUtils.info("Clear and Fill " + loggedValue + " on " + by.toString());

      if (ExtentTestManager.getExtentTest() != null) {
         ExtentReportManager.pass("Clear and Fill " + loggedValue + " on " + by);
      }
      AllureManager.saveTextLog("Clear and Fill " + loggedValue + " on " + by);
   ...
   /**
    * Click on Elements on the web with Javascript (click implicitly without fear
    * of being hidden)
    *
    * @param by an element of object type By
    */
   ...
   public static void clickElementWithJs(WebElement element) {
      ...
      LogUtils.info("Click on element with JS: " + element);
      if (ExtentTestManager.getExtentTest() != null) {
         ExtentReportManager.pass("Click on element with JS: " + element);
      }
      AllureManager.saveTextLog("Click on element with JS: " + element);
      addScreenshotToReport(
            Thread.currentThread().getStackTrace()[1].getMethodName() + "_" + DateUtils.getCurrentDateTime());
   }
   ...
   /**
    * Check the value of each column of the table when searching according to EQUAL
    * conditions (equals)
    *
   ...
   /**
    * Check the value of each column of the table when searching according to the
   ...
   /**
    * Check the value of each column of the table when searching according to the
    * CONTAINS condition with custom xpath
    *
    * @param column           column position
    * @param value            value to compare
   ...
   // DriverManager.getDriver().findElements(By.xpath("//tbody/tr"));
   // sleep(1);
   // LogUtils.info("Number of results for column (" + column + "): " +
   // totalRows.size()); //Không thích ghi log thì xóa nhen
   //
   ...
   // boolean res = false;
   // WebElement title =
   // DriverManager.getDriver().findElement(By.xpath("//tbody/tr[" + i + "]/td[" +
   // column + "]"));
   // arrayList.add(title.getText());
   // LogUtils.info("Row " + i + ":" + title.getText()); //Không thích ghi log thì
   // xóa nhen
   // }
   // }
   //
   ...
   public static WebElement waitForElementVisible(By by, int timeout) {
      try {
         ...
         // Cuộn tới giữa màn hình một cách êm ái để chuẩn bị click/action (Tránh gắp
         // trúng Menu Header che khuất)
   ...
   /**
    * Wait for the given element to be clickable within the given time (in
    * seconds).
    *
    * @param by      an element of object type By
   ...
   public static boolean waitForElementInvisible(By by, int timeOut) {
      try {
         WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(timeOut));
   ...
   public static boolean waitForElementInvisible(By by) {
      return waitForElementInvisible(by, FrameworkConstants.WAIT_EXPLICIT);
   }
   ...
   public static boolean verifyElementAttributeValue(By by, String attributeName, String attributeValue) {
      ...
      try {
         ...
         LogUtils.error(
               "Element: " + by.toString() + ". Not found Attribute value: " + attributeValue + " with attribute name: "
                     + attributeName + ". Actual get Attribute value is: " + getAttributeElement(by, attributeName));
         Assert.fail(
               "Element: " + by.toString() + ". Not found Attribute value: " + attributeValue + " with attribute name: "
                     + attributeName + ". Actual get Attribute value is: " + getAttributeElement(by, attributeName));
         return false;
   ...
   public static void waitForPageLoaded() {
      WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(),
            Duration.ofSeconds(FrameworkConstants.WAIT_PAGE_LOADED));
      JavascriptExecutor js = (JavascriptExecutor) DriverManager.getDriver();

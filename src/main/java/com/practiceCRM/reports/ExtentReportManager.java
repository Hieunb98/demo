...
public class ExtentReportManager {
    ...
    public static void initReports() {
        if (Objects.isNull(extentReports)) {
            ...
            try {
                ExtentPDFReporter pdf = new ExtentPDFReporter("exports/ExtentReports/ExtentReports.pdf");
                File pdfConfigFile = new File("src/test/resources/pdf-config.json");
                if (pdfConfigFile.exists()) {
                    pdf.loadJSONConfig(pdfConfigFile);
                }
                extentReports.attachReporter(pdf);
                LogUtils.info("Extent PDF Reporter is attached.");
            } catch (Exception e) {
                LogUtils.warn("Không thể cấu hình ExtentPDFReporter: " + e.getMessage());
            }
    ...
    public static void addScreenShot(String message) {
        ...
        // ExtentTestManager.getExtentTest().log(Status.INFO,
        // MediaEntityBuilder.createScreenCaptureFromPath(String.valueOf(CaptureHelpers.getScreenshotFile(message))).build());
    ...
    public static void addScreenShot(Status status, String message) {
        ...
        // File Path from Screenshot of Java
        // ExtentTestManager.getExtentTest().log(status,
        // MediaEntityBuilder.createScreenCaptureFromPath(CaptureHelpers.getScreenshotAbsolutePath(message)).build());

    }

    synchronized public static void addAuthors(AuthorType[] authors) {
        if (authors == null) {
            ExtentTestManager.getExtentTest().assignAuthor("Bat Hieu");
        } else {
    ...
    synchronized public static void addDevices() {
        ...
        // .assignDevice(BrowserIconUtils.getBrowserIcon() + " : " +
    ...
    public static void pass(String message) {
        // LogUtils.info("ExtentReportManager class: " +
